...
runCommand "synology-dev-root" { } ''
    ...
    install -Dm644 ${../config/synology-dev/grok/config.toml} "$out/etc/dotfiles/grok/config.toml"
    mkdir -p "$out/var/lib/fail2ban" "$out/var/run/fail2ban" "$out/etc/profile.d"
    install -Dm755 ${../scripts/synology-dev-workspace-novnc.sh} "$out/bin/synology-dev-workspace-novnc"
    cat > "$out/etc/profile.d/grok-litellm-harness.sh" <<'EOF'
  # LiteLLM virtual keys for Grok Build (synology-dev)
  if [ -f "$HOME/.local/state/grok-litellm-harness/virtual-keys.env" ]; then
    set -a
  ...
  EOF
    chmod 644 "$out/etc/profile.d/grok-litellm-harness.sh"
